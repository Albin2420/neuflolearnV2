    ...
    if (hidecontrols > 0) {
      ...
      switch (vlcPlayerCtrl.value.playingState) {
          ...
          if (widget.isLive) {
            ...
            await vlcPlayerCtrl.pause().then((_) {
              setState(() {
                ...
                live_backupPosition = backup;
                log("live_backupPosition:$live_backupPosition");
              });
            }).catchError((e) {
              log("Error in : $e");
            });
        ...
        case PlayingState.paused:
          log("Resuming play...");
          if (widget.isLive) {
            await vlcPlayerCtrl.seekTo(live_backupPosition).then((_) async {
              log("seeking to live_backupPosition:$live_backupPosition");
